...
const HomeFormSection = () => {
    ...
    const validateForm = () => {
        ...
        else if (!/^[A-Za-z\s]+$/.test(formData.name.trim())) newErrors.name = "Enter a valid name";

        if (!formData.phone.trim()) newErrors.phone = "Phone number is required";
        else if (!/^\d{10,14}$/.test(formData.phone.replace(/\D/g, ''))) newErrors.phone = "Enter a valid phone number";

        if (!formData.email.trim()) newErrors.email = "Email is required";
        else if (!/^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/.test(formData.email.trim())) newErrors.email = "Enter a valid email";
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        alert(`❌ Failed: ${res.data.message || "Something went wrong"}`);
      }
    } catch (error) {
      console.error(error);
